(function() {
  const fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      const force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      const element = document.getElementById("946dbb7b-c817-4196-bc2e-651626db7107");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '946dbb7b-c817-4196-bc2e-651626db7107' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      const js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-gl-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.4.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-mathjax-2.4.0.min.js"];
      const css_urls = [];
      
    
      const inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            const fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  const docs_json = '{"7e795ee1-ffd7-476c-8e1b-47457d61666b":{"defs":[],"roots":{"references":[{"attributes":{"data":{},"selected":{"id":"17939"},"selection_policy":{"id":"17938"}},"id":"17922","type":"ColumnDataSource"},{"attributes":{},"id":"17934","type":"BasicTickFormatter"},{"attributes":{"callback":null},"id":"17904","type":"HoverTool"},{"attributes":{},"id":"17900","type":"WheelZoomTool"},{"attributes":{},"id":"17897","type":"ResetTool"},{"attributes":{},"id":"17935","type":"AllLabels"},{"attributes":{"coordinates":null,"group":null,"items":[{"id":"17943"}]},"id":"17942","type":"Legend"},{"attributes":{},"id":"17902","type":"UndoTool"},{"attributes":{"coordinates":null,"data_source":{"id":"17922"},"glyph":{"id":"17923"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17925"},"nonselection_glyph":{"id":"17924"},"view":{"id":"17927"}},"id":"17926","type":"GlyphRenderer"},{"attributes":{"line_alpha":0.2,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17919","type":"Line"},{"attributes":{},"id":"17964","type":"Selection"},{"attributes":{},"id":"17883","type":"DataRange1d"},{"attributes":{"source":{"id":"17916"}},"id":"17921","type":"CDSView"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17917","type":"Line"},{"attributes":{},"id":"17963","type":"UnionRenderers"},{"attributes":{},"id":"17939","type":"Selection"},{"attributes":{},"id":"17931","type":"BasicTickFormatter"},{"attributes":{"axis":{"id":"17893"},"coordinates":null,"dimension":1,"group":null,"ticker":null},"id":"17896","type":"Grid"},{"attributes":{"source":{"id":"17922"}},"id":"17927","type":"CDSView"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"17937"},"selection_policy":{"id":"17936"}},"id":"17916","type":"ColumnDataSource"},{"attributes":{"coordinates":null,"data_source":{"id":"17944"},"glyph":{"id":"17945"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17947"},"nonselection_glyph":{"id":"17946"},"view":{"id":"17949"}},"id":"17948","type":"GlyphRenderer"},{"attributes":{"toolbar":{"id":"17969"},"toolbar_location":"above"},"id":"17970","type":"ToolbarBox"},{"attributes":{"overlay":{"id":"17906"}},"id":"17901","type":"LassoSelectTool"},{"attributes":{},"id":"17887","type":"LinearScale"},{"attributes":{"bottom_units":"screen","coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"17905","type":"BoxAnnotation"},{"attributes":{"fill_color":{"value":"#2a2eec"},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17945","type":"Circle"},{"attributes":{},"id":"17890","type":"BasicTicker"},{"attributes":{},"id":"17885","type":"LinearScale"},{"attributes":{"coordinates":null,"data_source":{"id":"17916"},"glyph":{"id":"17917"},"group":null,"hover_glyph":null,"muted_glyph":{"id":"17919"},"nonselection_glyph":{"id":"17918"},"view":{"id":"17921"}},"id":"17920","type":"GlyphRenderer"},{"attributes":{},"id":"17937","type":"Selection"},{"attributes":{},"id":"17898","type":"PanTool"},{"attributes":{"toolbars":[{"id":"17907"}],"tools":[{"id":"17897"},{"id":"17898"},{"id":"17899"},{"id":"17900"},{"id":"17901"},{"id":"17902"},{"id":"17903"},{"id":"17904"}]},"id":"17969","type":"ProxyToolbar"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"17918","type":"Line"},{"attributes":{"children":[{"id":"17970"},{"id":"17968"}]},"id":"17971","type":"Column"},{"attributes":{"fill_alpha":{"value":0.2},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.2},"line_alpha":{"value":0.2},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17947","type":"Circle"},{"attributes":{"axis":{"id":"17889"},"coordinates":null,"group":null,"ticker":null},"id":"17892","type":"Grid"},{"attributes":{"line_alpha":0.2,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17925","type":"Line"},{"attributes":{},"id":"17932","type":"AllLabels"},{"attributes":{},"id":"17903","type":"SaveTool"},{"attributes":{"coordinates":null,"formatter":{"id":"17931"},"group":null,"major_label_policy":{"id":"17932"},"ticker":{"id":"17894"}},"id":"17893","type":"LinearAxis"},{"attributes":{},"id":"17938","type":"UnionRenderers"},{"attributes":{"coordinates":null,"group":null,"text":"y / y","text_font_size":"15pt"},"id":"17950","type":"Title"},{"attributes":{},"id":"17936","type":"UnionRenderers"},{"attributes":{"overlay":{"id":"17905"}},"id":"17899","type":"BoxZoomTool"},{"attributes":{"label":{"value":"bpv=0.53"},"renderers":[{"id":"17926"}]},"id":"17943","type":"LegendItem"},{"attributes":{"fill_alpha":{"value":0.1},"fill_color":{"value":"#2a2eec"},"hatch_alpha":{"value":0.1},"line_alpha":{"value":0.1},"size":{"value":6.0},"x":{"value":-1.135840820153207},"y":{"value":0}},"id":"17946","type":"Circle"},{"attributes":{"tools":[{"id":"17897"},{"id":"17898"},{"id":"17899"},{"id":"17900"},{"id":"17901"},{"id":"17902"},{"id":"17903"},{"id":"17904"}]},"id":"17907","type":"Toolbar"},{"attributes":{"coordinates":null,"formatter":{"id":"17934"},"group":null,"major_label_policy":{"id":"17935"},"ticker":{"id":"17890"}},"id":"17889","type":"LinearAxis"},{"attributes":{"data":{},"selected":{"id":"17964"},"selection_policy":{"id":"17963"}},"id":"17944","type":"ColumnDataSource"},{"attributes":{"source":{"id":"17944"}},"id":"17949","type":"CDSView"},{"attributes":{"below":[{"id":"17889"}],"center":[{"id":"17892"},{"id":"17896"},{"id":"17942"}],"height":500,"left":[{"id":"17893"}],"output_backend":"webgl","renderers":[{"id":"17920"},{"id":"17926"},{"id":"17948"}],"title":{"id":"17950"},"toolbar":{"id":"17907"},"toolbar_location":null,"width":500,"x_range":{"id":"17881"},"x_scale":{"id":"17885"},"y_range":{"id":"17883"},"y_scale":{"id":"17887"}},"id":"17880","subtype":"Figure","type":"Plot"},{"attributes":{},"id":"17894","type":"BasicTicker"},{"attributes":{},"id":"17881","type":"DataRange1d"},{"attributes":{"coordinates":null,"fill_alpha":0.5,"fill_color":"lightgrey","group":null,"level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"17906","type":"PolyAnnotation"},{"attributes":{"line_alpha":0,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17923","type":"Line"},{"attributes":{"children":[[{"id":"17880"},0,0]]},"id":"17968","type":"GridBox"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"17924","type":"Line"}],"root_ids":["17971"]},"title":"Bokeh Application","version":"2.4.0"}}';
                  const render_items = [{"docid":"7e795ee1-ffd7-476c-8e1b-47457d61666b","root_ids":["17971"],"roots":{"17971":"946dbb7b-c817-4196-bc2e-651626db7107"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    let attempts = 0;
                    const timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (let i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();